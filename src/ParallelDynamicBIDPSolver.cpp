...
#include "enmod/ParallelBIDP.h" // Uses the CUDA solver
#include "enmod/Logger.h"
#include <algorithm>
#include <cmath>

ParallelDynamicBIDPSolver::ParallelDynamicBIDPSolver(const Grid& grid_ref) 
    : Solver(grid_ref, "ParallelDynamicBIDPSim"), current_mode(EvacuationMode::NORMAL) {}

void ParallelDynamicBIDPSolver::assessThreatAndSetMode(const Position& current_pos, const Grid& current_grid) {
...
void ParallelDynamicBIDPSolver::run() {
    ...
    for (int t = 0; t < 2 * (dynamic_grid.getRows() * dynamic_grid.getCols()); ++t) {
        ...
        // --- GPU ACCELERATION HERE ---
        ParallelBIDP step_planner(dynamic_grid);
        step_planner.run();
...
Cost ParallelDynamicBIDPSolver::getEvacuationCost() const { return total_cost; }

void ParallelDynamicBIDPSolver::generateReport(std::ofstream& report_file) const {
    report_file << "<h2>Simulation History (Parallel Dynamic BIDP - CUDA)</h2>\n";
